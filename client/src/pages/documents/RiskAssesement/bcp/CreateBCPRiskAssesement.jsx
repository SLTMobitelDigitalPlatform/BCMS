...
const CreateBCPRiskAssesement = () => {
  ...
  const fetchBCObjectives = async () => {
    try {
      const response = await axios.get("http://localhost:5000/objectives");
      const bcOptions = response.data.map((item) => item.businessContinuity);
      setBusinessContinuityOptions(bcOptions);
  ...
  const handleaddDoc = (e) => {
    ...
    axios
      .post("http://localhost:5000/api/risksBCP/add", data)
      .then(() => {
        handleSuccessAlert();
  ...
  return (
    ...
    <div className="flex gap-x-10 h-full overflow-y-auto bg-sky-100 rounded-2xl">
      <div className=" w-full">
        <h1 className="text-2xl font-bold">Add New BCP Risk Assesement</h1>
            ...
            <div className="flex flex-col gap-2">
              ...
              <div className="flex flex-col gap-2">
                ...
                >
                  ...
                  {businessContinuityOptions.map((option, index) => (
                    <option key={index} value={option}>
                      {option}
                    </option>
                  ))}
...
export default CreateBCPRiskAssesement;
