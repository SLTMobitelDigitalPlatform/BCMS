    ...
    <div className="overflow-x-auto pb-40">
      <table className="table-fixed relative min-w-max p-20 border-collapse border border-indigo-100">
        <thead className="sticky text-white h-16">
          <tr>
            <th className="w-36 sticky left-0 bg-indigo-800 border-indigo-100">Site</th>
            {headers.map((header) => (
              <th key={header} className="w-20 border border-indigo-100 text-center bg-indigo-800">{header}</th>
            ))}
          </tr>
        </thead>
        <tbody>
          {rows.map((row, rowIndex) => (
            <tr key={rowIndex}>
              <td className="sticky left-0 h-24 px-6 py-4 border border-indigo-100 bg-indigo-100">{row}</td>
...
const Manpower = () => {
  return (
    <div className="p-8">
      <h1 className="text-xl font-bold mb-4">Minimum Operating Requirements - Manpower/Manpower</h1>
      <ImpactAnalysisGrid />
    </div>
  );
};
