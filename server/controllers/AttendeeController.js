const Attendee = require("../models/AttendeeModel.js");
...
const createAttendees = async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ message: "Failed to store Attendee details", error });
  }
};

// get attendee data for single meeting
const getsingleAttendeeData = (req, res) => {
  Attendee.find({ meetingId: req.params.meetingId }).populate("attendeeId")
